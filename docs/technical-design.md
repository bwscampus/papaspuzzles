...
| `reviewPuzzle(id, accept\|reject\|restore)` / `syncBatchStatus`                  | one donated puzzle → available (+1) / rejected (−1 if credited) / available again; batch status follows its puzzles (pending while any pending, else accepted if any kept, else rejected); `credits_awarded` = ledger sum for the batch |
| `rejectDonationBatch(id)`                                                       | remaining pending puzzles → rejected; then `syncBatchStatus`                                                                                                                                                                                                                                                                                                          |
...
| POST `/api/admin/donation-batches/[id]`                           | A    | `{action: 'accept' \| 'reject'}` (applies to puzzles still pending)                     | `{creditsAwarded, puzzlesPublished}`                                                      |
| POST `/api/admin/puzzles/[id]/review`                             | A    | `{action: 'accept' \| 'reject' \| 'restore'}` (donated puzzles only)                     | `{puzzle, balance}`                                                                       |
